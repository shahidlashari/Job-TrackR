...
// }
class HistogramrChart extends Component {
  ...
  render() {
    ...
    return (
      <>
        <Container>
          {this.props.histogram?.map((x, idx) => {
            return (
            // eslint-disable-next-line react/no-array-index-key
              <List.Item key={idx}>
                <List.Content>
                  <List.Header>{x.location.display_name}</List.Header>
                  <List.Header>{x.histogram}</List.Header>
...
export default connect(mapStateToProps, {})(HistogramrChart);
